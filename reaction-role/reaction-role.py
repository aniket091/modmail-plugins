...
class ReactionRoles(commands.Cog):
    ...
    async def rr_add(self, ctx, message: str, role: discord.Role, emoji: Emoji,
        ...
        - **Note(s):**
        You can only use the emoji once, you can't use the same emoji multiple times.
        - **Usage:**
        ...
        embed = discord.Embed(description="**✅ Successfuly setup the Reaction Role!**", color = 0x00ff5a)
        await ctx.send(embed = embed)
        
    @reactionrole.command(name="remove")
    @checks.has_permissions(PermissionLevel.ADMINISTRATOR)
    async def rr_remove(self, ctx, emoji: Emoji):
        """Delete a reaction role from the list."""
        emote = emoji.name if emoji.id is None else str(emoji.id)
        ...
        embed = discord.Embed(description="**✅ Successfully removed the role from the reaction role!**", color = 0x00ff5a)
        await ctx.send(embed = embed)
    ...
    async def rr_lock(self, ctx, emoji: Emoji):
        ...
        **Example:**
        ...
        embed = discord.Embed(description="**✅ Succesfully locked the reaction role!**", color = 0x00ff5a)
        await ctx.send(embed = embed)
    ...
    async def rr_unlock(self, ctx, emoji: Emoji):
        ...
        await self.db.find_one_and_update(
        {"_id": "config"}, {"$set": {emote: config[emote]}}, upsert=True)
        embed = discord.Embed(description="**✅ Succesfully unlocked the reaction role!**", color = 0x00ff5a)
        await ctx.send(embed = embed)
